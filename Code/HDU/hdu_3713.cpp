//2012-09-21 10:19:01	Accepted	3713	0MS	316K	3912 B	G++	Aros
#include<cstdio>
#include<cstring>
#include<algorithm>
#include<queue>
...
char stepto[MAXN][MAXN][MAXN][MAXN], move[] = {'D','L','R','U'};
    ...
    while (--T)
    {
        ...
        while (!Q.empty())
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                {
                    Q.push(v);
                    stepto[v.first.first][v.first.second][v.second.first][v.second.second] = move[i];
